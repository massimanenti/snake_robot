...
% OUTPUT
% v           =   [F;h];
% F represents the cost function value, Gauss-Newton case.
% h represents the nonlinear inequality constraints
% 
% INPUT
% serpenoid_curve_parameters, optimization variables (alpha, omega, beta, gamma)
...
for ind_vary=1:n_vary       % ind_vary: index to select the time intervals
    ...
    for t=T(1,ind_vary)+1:T(1,ind_vary+1)   % t: index to select the time istants
        ...
        % needed here        
        [zdot, Xj, Yj] = model_simulation_snake(0,zsim(:,t-1), u, 0, parameters);
